...
$email = $data['email'];
$pinjaman_pokok = $data['jmlpengajuan'];
$nama = $data['nama'];
$jasa = '1.5';
...
$subjek = "Selamat Permohonan Anda Telah diterima"; // Ambil subjek dari inputan form
// $pesan = $_POST['pesan']; // Ambil pesan dari inputan form
$pesan = "Selamat Pengajuan anda diterima, silahkan ke Kantor BUMDes dengan membawa berkas sebagai berikut : <br> 
1. Foto Copy KK
2. Foto Copy KTP
3. Surat Keterangan Usaha Asli
";
...
if(empty($attachment)){ // Jika tanpa attachment
    ...
    $update = "UPDATE permohonan_pinjaman SET validasi = 'Diterima' WHERE id_perpinjaman = '".$id."' ";

    $totaljasa = $pinjaman_pokok * $jasa / 100;
    $jasa15 = $totaljasa * 10;
    $totalbayar = $jasa15 + $pinjaman_pokok;

    $query = mysqli_query($koneksi, 'INSERT INTO anggota (nama, id_perpinjaman, pinjaman_pokok, jasa, totaljasa, totalbayar) VALUE ("'.$nama.'", "'.$id.'", "'.$pinjaman_pokok.'", "'.$jasa.'", "'.$jasa15.'", "'.$totalbayar.'")') or die(mysqli_error($koneksi));
